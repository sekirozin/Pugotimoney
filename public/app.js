...
function updateProfileInfo() {
    ...
    badge.style.background = user.role === 'admin' ? '#f5576c' : 'var(--primary)';
}
...
class FinanceApp {
    ...
    updateDashboard() {
        const periodBalance = this.calculatePeriodBalance(this.selectedMonth, this.selectedYear);
        ...
        const savingsGoal = parseFloat(localStorage.getItem('savingsGoal')) || 1000;
        ...
        document.getElementById('savings-goal').textContent = `R$ ${savingsGoal.toFixed(2)}`;
    ...
    async addCreditCard() {
        ...
        const editId = document.getElementById('card-modal').dataset.editId;
        ...
        try {
            ...
            if (editId) {
                ...
                if (response.ok) {
                    const idx = app.creditCards.findIndex(c => c.id == editId);
                    if (idx !== -1) app.creditCards[idx] = { ...card, id: parseInt(editId) };
    ...
    loadInstallments() {
        ...
        if (this.installments.length === 0) {
            list.innerHTML = '<p style="text-align: center; color: var(--muted);">Nenhum parcelamento cadastrado</p>';
            return;
        }
        ...
        this.installments.forEach(inst => {
            ...
            const installmentDueThisMonth = monthsSinceStart >= 0 && monthsSinceStart < totalInstallments;
            const currentMonthInstallment = monthsSinceStart + 1;
            const isCurrentMonthPaid = isCompleted || (installmentDueThisMonth && paidInstallments >= currentMonthInstallment);
            const monthStatusClass = isCurrentMonthPaid ? 'paid' : installmentDueThisMonth ? 'open' : 'upcoming';
            const monthStatusIcon = isCurrentMonthPaid ? 'check-circle-2' : installmentDueThisMonth ? 'circle-alert' : 'calendar-clock';
            const monthStatusLabel = isCompleted
                ? 'Quitado'
                : isCurrentMonthPaid
                    ? 'Fatura do mês paga'
                    : installmentDueThisMonth
                        ? 'Fatura do mês em aberto'
                        : 'Aguardando início';

            const item = document.createElement('div');
            item.className = `card ${isCurrentMonthPaid ? 'installment-paid-card' : ''}`;
            item.style.marginBottom = '15px';
            item.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: start;">
                    <div style="flex: 1;">
                        <h3>${esc(inst.description)}</h3>
                        <div class="installment-month-status ${monthStatusClass}">
                            <i data-lucide="${monthStatusIcon}" class="lucide-icon"></i>
                            <span>${monthStatusLabel}</span>
                        </div>
                    ...
                    <div style="text-align: right;">
                        <span style="display: inline-block; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: bold; background: ${isCompleted ? 'var(--success)' : 'var(--warning)'}; color: white;">
                ...
                <div style="margin-top: 10px;">
                    <div style="background: var(--border); height: 8px; border-radius: 4px; overflow: hidden;">
                        <div style="background: ${isCompleted ? 'var(--success)' : 'var(--primary)'}; height: 100%; width: ${progress}%;"></div>
                    </div>
    ...
    loadSubscriptions() {
        ...
        summary.style.background = 'linear-gradient(135deg, var(--primary-strong) 0%, var(--primary) 100%)';
