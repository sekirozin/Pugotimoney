...
const DEFAULT_SECTIONS = ['installments', 'expenses', 'purchases', 'shopping', 'budgets', 'income', 'cards', 'goals'];
...
function addTotal(doc, label, value) {
    ensureSpace(doc, 24);
...
class ReportService {
    static async generateMonthlyStatement(year, month, sections = DEFAULT_SECTIONS) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const shoppingItems = await database_1.database.getShoppingItems();
