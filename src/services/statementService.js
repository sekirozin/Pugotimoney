...
exports.previewStatement = previewStatement;
...
function parsePortugueseDate(value) {
    ...
    const normalized = value
        ...
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
    const match = normalized.match(/\b(\d{1,2})\s+de\s+([a-z]+)\s+(\d{4})\b/);
    if (!match)
        return null;
...
function inferTransactionType(amount, line) {
    if (amount < 0)
        return 'expense';
    ...
    if (expenseWords.some(word => normalized.includes(word)))
        return 'expense';
    if (incomeWords.some(word => normalized.includes(word)))
        return 'income';
    return 'income';
}
...
function parseStatementLines(lines) {
    if (lines.length < 2) {
        throw new Error('Extrato deve conter pelo menos 2 linhas');
    }
    const transactions = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (isPageNoise(line))
            continue;
        const pdfTransactionMatch = line.match(/^(\d{2}:\d{2})\s+(.+?)\s+([+−-]?\s*R?\$?\s*\d{1,3}(?:[.,]\d{3})*[.,]\d{2}|[+−-]?\s*R?\$?\s*\d+[.,]\d{2})\s*(.*)$/);
...
async function extractStatementText(buffer, mimeType, filename) {
...
async function processStatement(buffer, mimeType, filename, options = {}) {
    ...
    const existingTransactions = await database_1.database.getTransactions();
    ...
    for (const transaction of newTransactions) {
        await database_1.database.addTransaction(transaction);
    }
    return {
...
async function previewStatement(buffer, mimeType, filename) {
    const content = await extractStatementText(buffer, mimeType, filename);
    const lines = content.split(/\r?\n/).map(line => line.trim()).filter(Boolean);
