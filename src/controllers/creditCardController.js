...
exports.creditCardController = exports.CreditCardController = void 0;
...
class CreditCardController {
    ...
    async getCreditCards(req, res) {
        try {
            const cards = await database_1.database.getCreditCards();
            // Calcular limite disponível para cada cartão
            const cardsWithAvailableLimit = cards.map(card => ({
                ...card,
                availableLimit: (0, helpers_1.getCardAvailableLimit)(card)
            }));
            res.json({
                success: true,
    ...
    async addCreditCard(req, res) {
        try {
            ...
            // Validar dados
            const validation = (0, helpers_1.validateCreditCard)(cardData);
            ...
            const cardId = await database_1.database.addCreditCard(cardData);
            res.status(201).json({
                success: true,
    ...
    async updateCreditCard(req, res) {
        try {
            ...
            // Validar dados
            const validation = (0, helpers_1.validateCreditCard)(updates);
            if (!validation.isValid) {
                return res.status(400).json({
                    success: false,
                    error: 'Dados inválidos',
            ...
            const updated = await database_1.database.updateCreditCard(parseInt(id), updates);
            if (!updated) {
    ...
    async deleteCreditCard(req, res) {
        try {
            const { id } = req.params;
            const deleted = await database_1.database.deleteCreditCard(parseInt(id));
            if (!deleted) {
                return res.status(404).json({
                    success: false,
                    error: 'Cartão não encontrado'
    ...
    async getUpcomingInvoices(req, res) {
        try {
            const cards = await database_1.database.getCreditCards();
    ...
    async updateCardBalance(req, res) {
        try {
            const { id } = req.params;
            ...
            const updated = await database_1.database.updateCreditCard(parseInt(id), {
